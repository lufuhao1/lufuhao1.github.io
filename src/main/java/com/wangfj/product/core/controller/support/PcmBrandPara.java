 ...
 * 品牌参数
 * 
 * @Class Name PcmBrandPara
 * @Author wangx
 * @Create In 2015-8-13
 */
public class PcmBrandPara extends BasePara {

	/**
	 * @Field long serialVersionUID
	 */
	private static final long serialVersionUID = -1280227656010262771L;

	private Long sid;

	@NotNull(message = "{PcmBrandPara.brandName.isNotNull}")
	private String brandName;// 品牌名称

	@Pattern(regexp = "^[A-Za-z0-9\\s]{1,20}$", message = "{PcmBrandPara.spell.formatError}")
	private String spell;// 拼音

	private String pictureUrl;// 图片路径

	@Pattern(message = "{PcmBrandPara.brandNameSecond.formatError}", regexp = "^[0-9\\s\u4E00-\u9FA5]{1,20}$")
	private String brandNameSecond;// 中文名字
	...
	@NotNull(message = "{PcmBrandPara.shopType.isNotNull}")
	private String shopType;// 门店类型(0 北京，1 外埠 ，2 电商erp)默认为0

	@NotNull(message = "{PcmBrandPara.brandType.isNotNull}")
	private Integer brandType;// 品牌类型 0 集团品牌 1 门店品牌 （默认为0）

	private Integer status = 0;// 有效标记：0有效，1无效（默认为0）

	@Size(message = "{PcmBrandPara.brandDesc.size}", max = 500)
